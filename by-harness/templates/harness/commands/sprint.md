...
2. 若存在 `.harness/task-harness/index.json`，将 active bucket 中对应 feature 的 `passes` 更新为 `true`（若存在 `.harness/feature_list.json` 则同步兼容镜像）
3. 若存在 `.harness/task-harness/progress/latest.txt`，追加本轮冲刺记录
