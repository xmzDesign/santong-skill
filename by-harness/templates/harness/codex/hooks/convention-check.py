#!/usr/bin/env python3
"""
Codex hook/CLI: check Java/MyBatis SQL conventions on changed files.
"""
        ...
        if class_name.endswith("AppService") and not class_name.endswith("AppServiceImpl"):
            if class_kind != "interface":
                add_finding(
                    ...
                    path,
                    class_line,
                    "AppService 必须是接口；实现放在 XxxAppServiceImpl。",
        ...
        if class_name.endswith("AppServiceImpl"):
            ...
            if not re.search(rf"\bimplements\s+[^\{{;]*\b{re.escape(expected_interface)}\b", text):
                add_finding(
                    ...
                    path,
                    class_line,
                    f"{class_name} 必须实现 {expected_interface}。",
                    lines[class_line - 1] if lines else "",
                )
        if public_dto_file:
    ...
    for index, line in enumerate(lines):
        ...
        if re.search(r"@(Select|Update|Insert|Delete)\s*\(", line):
            add_finding(
                ...
                path,
                line_no,
                "MyBatis SQL 必须写在 XML Mapper，禁止写在 Java 注解中。",
                line,
            )
        ...
        if re.search(r"\bqueryForList\s*\([^;\n]+,\s*[^,\n]+,\s*[^)\n]+\)", line):
            add_finding(
                findings,
                "fail",
                "IBATIS_MEMORY_PAGING",
                root,
                path,
                line_no,
                "禁止使用 iBATIS queryForList(statementName, start, size) 做内存分页；分页必须下推到 SQL。",
                line,
            )
        if re.search(r"\b(HashMap|Hashtable)\s*<", line) and re.search(r"\b(Mapper|Dao|DAO|Repository)\b", path.name + " " + line):
            add_finding(
                ...
                path,
                line_no,
                "数据库查询结果禁止直接暴露 HashMap/Hashtable；请定义 DO/DTO 和 resultMap。",
                line,
            )
...
def format_text(findings: list[Finding], files: list[Path], root: Path) -> str:
    if not findings:
...
def emit_hook(findings: list[Finding], files: list[Path], root: Path):
    text = format_text(findings, files, root)
    fails = [item for item in findings if item.severity == "fail"]
    warns = [item for item in findings if item.severity == "warn"]
    if fails:
...
def main():
    ...
    if args.format == "json":
        ...
        emit_hook(findings, files, root)
    else:
        print(format_text(findings, files, root))
