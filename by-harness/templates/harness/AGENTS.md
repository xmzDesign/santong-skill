...
2. `plan <功能描述>`
3. `build <spec 文件或最新 spec>`
4. `qa <contract 文件或最新 contract>`
5. `sprint <功能描述>`（完整周期）
...
| 意图 | 触发示例 | 必要动作 | 必要输出 |
|---|---|---|---|
| 默认自然语言改动 | “修一下空指针”, “调整这个校验”, “优化错误提示”, “补一个单测” | 未显式指定 `plan/build/qa/sprint` 时，先运行快速分类器；quick_fix/fast_track 按结果执行，standard_feature 回到 sprint | 分类结果 + 代码改动或 sprint 工件 |
| `plan` | “plan login flow”, “write spec for X” | 自动归纳假设/歧义、检查相关代码、编写 spec | `.harness/docs/specs/<feature>.md` |
...
| `quick-fix` / `fast-track` | “修一个明确小 bug”, “局部调整校验规则” | 先运行快速分类器；high quick-fix 走轻量修复，high/medium fast-track 走局部快速通道，low 回到 sprint | 代码改动 + 定向验证 + 快速收口日志 |
...
即使用户没有显式写 `quick-fix` 或 `fast-track`，只要请求是修复、调整、优化、补测试、改校验、改提示或处理报错这类自然语言改动，且没有显式指定 `plan/build/qa/sprint`，也必须先运行分类器；不要直接进入完整 sprint。
...
- 未显式指定 `plan/build/qa/sprint` 的自然语言改动，若明确、局部、可验证，必须先用 quick-fix/fast-track 分流；快速模式只写日志，不绕过 feature 完成门禁。
- 禁止隐式扩范围。
