...
2. **先看本地实现**：新增模式前必须查看同层已有实现，优先复用已有实体、表、DTO、Service、工具、转换器、异常、响应包装、配置和命名。
3. **最小安全改动**：只修改完成任务所需的最小范围；历史项目小改动如无必要不新增实体、表、新层级或框架胶水。
...
6. SQL 位置已明确：MyBatis SQL 写在 XML Mapper，禁止写进 Java 注解。
7. SQL 语义已检查：count、NULL、分页、表别名、数据订正和回滚。
8. ORM 映射已检查：显式字段、`resultMap`、参数绑定、更新字段范围、`update_time`。
9. 定时任务已检查：调度任务使用 XXL-Job，不新增临时本地 Scheduler。
10. 数据迁移已检查：默认不做应用启动迁移，除非任务明确要求 schema 变更。
11. 分布式影响已声明：所有 Java 改动都要说明分布式 Java 门禁未触发或已触发。
