...
8. 分布式影响确认：所有 Java 改动必须声明是否触发第 14 章；凡涉及外部调用、并发、锁、缓存、消息、事务一致性、容错、可观测性、发布停机，逐项满足第 14 章。
...
所有 Java 代码同时默认受第 14 章《通用分布式编码契约》约束。即使本次只是普通接口、查询或领域逻辑，也必须在 spec/contract 中写明“Distributed Java Gate：不触发/触发”，并给出理由；不能省略分布式影响判断。
...
- 新增或修改外部调用、Dubbo/HTTP/RPC 客户端、MQ 生产/消费、异步任务、线程池、批量处理、事务边界、补偿逻辑、发布停机相关逻辑。
...
- Distributed Java Gate：必须声明“未触发”或列出触发的第 14 章条款：
  - 资源隔离与并发治理：线程池/队列/Worker 池/拒绝策略。
  - 超时、重试与幂等：外部调用、写接口、Job、消息消费。
  - 分布式锁与并发一致性：锁 key、等待时间、租约时间、持有者校验释放。
  - 事务与最终一致性：事务边界、跨服务一致性、补偿路径、人工接管。
  - 缓存治理：TTL、空值缓存、穿透/击穿/雪崩、降级策略。
  - 消息与事件处理：`traceId/messageId/业务键`、消费幂等、死信/失败表/重放。
  - 批量处理与异步落盘：有界队列、双刷盘、停机冲刷。
  - 容错、降级与补偿：熔断、降级、重放、终止、告警去重。
  - 可观测性与上下文传递：日志/指标/trace、异步 `MDC/ThreadLocal` 传递与清理。
  - 配置与安全边界：集中配置、审计回滚、鉴权限流、脱敏、密钥托管。
  - 发布、回滚与优雅停机：健康检查、灰度、回滚、在途任务处理。

如果 spec 没有 Java Gate 或 Distributed Java Gate，小范围任务也必须在 build 前补齐；不能用“代码改动很小”跳过。
...
- [ ] 已声明 Distributed Java Gate：未触发需说明原因；触发时逐条对照第 14 章。
- [ ] 外部调用具备超时、重试上限、幂等前提和退避策略。
- [ ] 并发资源、线程池、队列、Worker 池具备隔离、命名、容量与拒绝策略。
- [ ] 分布式锁具备命名空间、等待时间、租约时间、`finally` 释放和持有者校验。
- [ ] 事务边界最小化，跨服务一致性具备 Outbox/Saga/TCC/补偿路径。
- [ ] 消息生产/消费携带 `traceId/messageId/业务键`，消费端幂等，失败可补偿、可追踪、可重放。
- [ ] 异步边界正确透传并清理 `MDC/ThreadLocal`，关键链路指标可观测。
- [ ] 发布、回滚、优雅停机与在途任务处理有明确方案。
...
- 疑似硬编码密钥：fail。
- 新增/修改方法缺少中文注释：warn。
- 无界线程池、无容量队列、无超时锁、事务中疑似外部调用、消息缺少 `traceId/messageId`：warn/fail（按风险分级）。
...
- 所有 Java 改动已声明 Distributed Java Gate；触发分布式条款时，已逐条对照第 14 章并形成自检记录。
