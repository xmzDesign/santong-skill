...
2. **先本地后通用，勿增实体和魔法值**：先看本地已有实体、表、DTO、Service、配置、扩展点和命名；如无必要，不新增实体、表、新层级，也不新增散落的业务字符串/数字魔法值。
...
1. 最小安全变更，不无依据扩范围；历史项目小改动默认复用既有实体、表、DTO、Service 和配置。
2. 保持公共契约兼容，破坏性变更必须由任务明确要求。
3. 不重复造工具类、异常类、转换器、响应包装或框架胶水；业务状态、任务类型、动作类型、错误码、配置 key、阈值禁止散落魔法字符串/魔法数字。
...
- **Plan**：spec 必须列出触发维度、读取分片、每条门禁的实现方式、验收方式和自动检查项；同时写明最小实体与成本评估。
