...
def scan_java(root: Path, path: Path, findings: list[Finding]):
    ...
    if class_match:
        ...
        if class_name.endswith("AppService") and not class_name.endswith("AppServiceImpl"):
            add_finding(findings, "fail", "JAVA_SERVICE_INTERFACE_REQUIRED", root, path, class_line, "AppService must be an interface; put implementation in XxxAppServiceImpl.", lines[class_line - 1] if lines else "")
        if class_name.endswith("AppServiceImpl"):
            ...
            if not re.search(rf"\bimplements\s+[^\{{;]*\b{re.escape(expected_interface)}\b", text):
                add_finding(findings, "fail", "JAVA_SERVICE_IMPL_CONTRACT", root, path, class_line, f"{class_name} must implement {expected_interface}.", lines[class_line - 1] if lines else "")
