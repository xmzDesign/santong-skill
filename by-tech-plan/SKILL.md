...
| 类型 | 适用场景 | 输出重点                           |
| --- | --- |--------------------------------|
| L0 交付执行单 | 需求明确、单点改动、无架构取舍、无库表/接口契约/跨系统影响 | 改什么、怎么做、怎么验收  |
| L1 轻量技术方案 | 单系统或单模块，有少量流程、接口、配置或页面调整，但风险局部可控 | 目标、需求拆分、核心流程，数据设计 |
...
按推荐方案类型选择模板。L0/L1 的章节可以少，但必须覆盖“目标、改动、验收、上线/回滚、问题记录”；L2/L3 使用完整 9 章结构，一级标题保持不变。
...
| 改动项 | 说明 | 验证方式 | 负责人 |
| --- | --- | --- | --- |
| {页面/接口/配置/脚本/文档/代码模块} | {具体改动} | {如何验证} | {负责人} |
...
| 影响对象 | 改动方式 | 验证方式 | 风险判断 |
| --- | --- | --- | --- |
| {模块/接口/页面/配置} | {具体变化} | {单测/联调/回归/人工验收} | 高/中/低 |

**核心流程：**
{用简短步骤说明正常路径和必要异常路径；如有复杂分支再补 Mermaid。}
